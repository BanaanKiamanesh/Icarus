function [t, Y, Out] = RungeKutta4(odeFun, tSpan, dt, Y0)
    ...
    for k = 1:StepNum-1
        ...
        if exist('Out', 'var') == 0
            Out.U      = zeros(numel(Tmp.U), StepNum); 
            Out.Err    = zeros(numel(Tmp.Err), StepNum); 
            Out.RefSig = zeros(numel(Tmp.RefSig), StepNum); 

            Out.U(:, 1)      = Tmp.U;
            Out.Err(:, 1)    = Tmp.Err;
            Out.RefSig(:, 1) = Tmp.RefSig;
        else
            Out.U(:, k)      = Tmp.U;
            Out.Err(:, k)    = Tmp.Err;
            Out.RefSig(:, k) = Tmp.RefSig;
        end
